...
export const createPlacement = async (req, res) => {
  try {
    const { term, year, locations, study_year } = req.body;
    const placement_exists = await Placement.findOne({
      term,
      year,
      study_year,
    });
    ...
    const placementLocationAvailableSeatsMap = placementLocations.reduce(
      (result, location) => {
    ...
    const placements = students.map((student) => {
      ...
      const availableLocations = placementLocations.filter(
        (location) => prevPlacements.indexOf(location._id) === -1
      );
      ...
      while (true) {
        ...
        if (
          placementLocationAvailableSeatsMap[
            availableLocations[randomIndex]._id
          ] > 0
        ) {
          break;
        }
...
export const updatePlacement = async (req, res) => {
  try {
    ...
    await Promise.all(
      toUpdateArr.map(async (placementInfo) => {
        await Placement.updateOne(
          { "placements.studentId": placementInfo.studentId },
          {
            $set: {
              "placements.$.placementLocationId":
                placementInfo.placementLocationId || null,
...
export const confirmPlacement = async (req, res) => {
  try {
    ...
    const isAllStudentsAssigned = placements.every(
      (obj) => obj.placementLocationId
    );

    if (!isAllStudentsAssigned) {
      throw new Error(
        "Some students are not assigned with the placement location. Please assign the location for all students and try again."
      );
    }

    // Add placement and placement location to each student
    await Promise.all(
      placements.map(async ({ studentId, placementLocationId }) => {
